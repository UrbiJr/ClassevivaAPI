...
function Session(sessionId) {
    ...
    this.profile = () => {
        return this.request('https://web.spaggiari.eu/acc/app/default/me.php', {})
            .then(res => {
    ...
    this.schoolCode = () => {
        return this.request('https://web.spaggiari.eu/ber/app/default/compilazione_modulo.php', {})
            .then(res => {
                var $ = cheerio.load(res);
                var result = {};
                if ($('[name=sede_codice]').length) {
                    result.school_code = $('[name=sede_codice]').val();
                    return result;
                } else throw new Error('Invalid or expired session ID')
            })
    };
